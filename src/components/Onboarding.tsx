...
export function Onboarding({ settings, onComplete }: Props) {
  ...
  const [stateCode, setStateCode] = useState(
    (brand as Brand & { state?: string }).state || ''
  );
  const [mainCity, setMainCity] = useState(
    (brand as Brand & { mainCity?: string }).mainCity || ''
  );
  const [serviceCitiesText, setServiceCitiesText] = useState(
    ((brand as Brand & { serviceCities?: string[] }).serviceCities || []).join(', ')
  );
  const [serviceRadius, setServiceRadius] = useState<number>(
    Number((brand as Brand & { serviceRadius?: number }).serviceRadius || 25)
  );
  ...
  const [tireRepairProfit, setTireRepairProfit] = useState<number>(
    Number((settings as Settings & { tireRepairTargetProfit?: number }).tireRepairTargetProfit ||
      settings.servicePricing?.['Flat Tire Repair']?.minProfit || 90)
  );
  const [tireReplaceProfit, setTireReplaceProfit] = useState<number>(
    Number((settings as Settings & { tireReplacementTargetProfit?: number }).tireReplacementTargetProfit ||
      settings.servicePricing?.['Tire Replacement']?.minProfit || 110)
  );
  ...
  const [freeMiles, setFreeMiles] = useState<number>(
    Number((settings as Settings & { freeMilesIncluded?: number }).freeMilesIncluded || 5)
  );
  ...
  const finish = async () => {
    if (!businessName.trim()) {
      addToast('Business name required', 'warn');
      setStep(1);
      return;
    }
    if (!stateCode || !mainCity.trim()) {
      addToast('State and main city required', 'warn');
      setStep(2);
      return;
    }
    setBusy(true);
    try {
      const serviceCities = serviceCitiesText
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean);
      ...
      // Merge updated profit targets into existing servicePricing while keeping
      // any other pricing the user already configured.
      const sp: Record<string, ServicePricing> = {
        ...(settings.servicePricing || {}),
      };
      const updatePricing = (key: string, profit: number) => {
        const prev = sp[key] || { enabled: true, basePrice: profit, minProfit: profit };
        sp[key] = { ...prev, enabled: true, minProfit: profit };
      };
      updatePricing('Flat Tire Repair', tireRepairProfit);
      updatePricing('Tire Replacement', tireReplaceProfit);
      updatePricing('Tire Installation', installationProfit);
          ...
          {step === 2 && (
            <div className="onboarding-step page-enter">
              ...
              <div className="field" style={{ marginTop: 14 }}>
                ...
                <input
                  value={serviceCitiesText}
                  onChange={(e) => setServiceCitiesText(e.target.value)}
                  placeholder="Hollywood, Hialeah, Miramar"
                />
                <div style={{ fontSize: 11, color: 'var(--t3)', marginTop: 4 }}>
                  Comma-separated. Used in invoices and review requests.
                </div>
              </div>
            ...
            <div className="onboarding-step page-enter">
              ...
              <div className="field-row">
                <div className="field">
                  ...
                  <input
                    type="number"
                    inputMode="decimal"
                    value={tireRepairProfit}
                    onChange={(e) => setTireRepairProfit(Number(e.target.value))}
                  />
                </div>
                <div className="field">
                  ...
                  <input
                    type="number"
                    inputMode="decimal"
                    value={tireReplaceProfit}
                    onChange={(e) => setTireReplaceProfit(Number(e.target.value))}
                  />
                </div>
              </div>
              ...
              <div className="field">
                ...
                <input
                  type="number"
                  inputMode="decimal"
                  value={installationProfit}
                  onChange={(e) => setInstallationProfit(Number(e.target.value))}
                />
              </div>
            </div>
          )}
            ...
            <div className="onboarding-step page-enter">
              ...
              <div className="field-row">
                ...
                <div className="field">
                  ...
                  <input
                    ...
                    inputMode="numeric"
                    value={freeMiles}
                    onChange={(e) => setFreeMiles(Number(e.target.value))}
                  />
                </div>
              </div>
              ...
              <div className="onboarding-summary">
                ...
                <div className="onboarding-summary-row">
                  ...
                  <strong>{mainCity}{stateCode ? `, ${stateCode}` : ''}</strong>
                </div>
                <div className="onboarding-summary-row">
                  <span>Weekly goal</span>
                  <strong>${weeklyGoal.toLocaleString()}</strong>
                </div>
                <div className="onboarding-summary-row">
                  <span>Profit targets</span>
                  <strong>${tireRepairProfit} / ${tireReplaceProfit} / ${installationProfit}</strong>
                </div>
                <div className="onboarding-summary-row">
                  <span>Travel</span>
                  <strong>${costPerMile.toFixed(2)}/mi · {freeMiles} free</strong>
                </div>
              </div>
            </div>
          )}
