...
export function InstallBanner() {
  ...
  useEffect(() => {
    ...
    if (ios) {
      setIsIOS(true);
      // Delay so the banner doesn't compete with the first paint or onboarding.
  ...
  return (
    // Positioned ABOVE the sticky save footer so it never overlaps primary actions
    // or dropdowns. Uses CSS var --install-banner-bottom which the page can adjust
    // when a sticky footer is active.
    <div className="install-banner" role="dialog" aria-label="Install app">
      ...
      <div className="install-banner-actions">
        <button className="btn xs secondary" onClick={dismiss} aria-label="Dismiss">
          Later
        </button>
        <button className="btn xs primary" onClick={install}>
          {isIOS ? 'How' : 'Install'}
        </button>
      </div>
    </div>
