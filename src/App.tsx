...
function AuthenticatedApp({ user }: { user: User }) {
  ...
  // Onboarding gate: every new business must complete the wizard before
  // reaching the dashboard. Persisted via brand.onboardingComplete in Firestore.
  if (!onboardingComplete) {
    return (
      <Onboarding
        settings={settings}
        onComplete={async (brandPatch, settingsPatch) => {
          if (Object.keys(settingsPatch).length) {
            await persistSettings(settingsPatch);
          }
          await updateBrand(brandPatch);
          addToast('Setup complete — welcome!', 'success');
        }}
      />
    );
  }
