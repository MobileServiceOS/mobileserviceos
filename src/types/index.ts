...
//  Notifications
//
//  Stored at `businesses/{bid}/notifications/{id}`. Read by everyone
//  in the business (dispatch board needs visibility), filtered to
//  the current user via the optional `targetUid` field. Written by:
//    - Job-save flows (assigned-tech notification)
//    - Payment-update flows (owner notification)
//    - Inventory listeners (low-stock alerts)
//    - Weekly-goal milestone checks
//
//  Each kind carries a `routeTo` hint the in-app notification center
//  uses to deep-link to the relevant tab + entity. No FCM / native
//  push for v1 — pure in-app surface backed by the existing Firestore
//  subscription. When the user wants Web Push later, the same docs
//  drive both surfaces; only the delivery layer changes.
// ─────────────────────────────────────────────────────────────────────

export type NotificationKind =
  | 'job_assigned'        // owner / admin assigned a job to a tech
  | 'payment_received'    // job's paymentStatus → Paid
  | 'low_inventory'       // a tire size dropped below the threshold
  | 'weekly_goal'         // weekly goal milestone hit / approaching
  | 'invoice_generated'   // invoice PDF was generated for a job
  | 'system';             // catch-all for future announcements

export type NotificationCategory = 'job' | 'payment' | 'inventory' | 'goal' | 'system';

export const NOTIFICATION_CATEGORY_BY_KIND: Record<NotificationKind, NotificationCategory> = {
  job_assigned:      'job',
...
export const NOTIFICATION_CATEGORIES: NotificationCategory[] = ['job', 'payment', 'inventory', 'goal', 'system'];
...
export interface NotificationDoc {
  ...
  /** Single-line title shown in the notification center + native
   *  banner. Keep under 60 chars. */
  title: string;
  /** Optional second line — more context the user can scan without
   *  tapping. Keep under 120 chars. */
  body?: string;
  /** ISO timestamp the notification fired. Drives sort + relative
   *  timestamp rendering. */
  createdAt: string;
  /** uid this notification targets. When undefined, it's a
   *  business-wide notification visible to every member. */
  targetUid?: string;
  /** Per-user read state. Stored as a map of uid → ISO timestamp the
   *  user marked it read; absent uid means unread for that user. */
  readBy?: Record<string, string>;
  /** Deep-link hint — { tab, entityId } the notification center
   *  uses to route on tap. */
  routeTo?: {
    tab: TabId;
    /** Job id, customer key, inventory id, etc. */
    entityId?: string;
  };
  /** Optional small metadata bag (e.g. tire size, amount, etc.) for
   *  the notification center to render extra context. */
  meta?: Record<string, string | number | boolean | null>;
}

/**
 * Per-user notification preferences. Stored at
 * `businesses/{bid}/notificationPrefs/{uid}` — each user can write
 * only their own doc. When absent, every category is on by default.
 */
export interface NotificationPrefs {
  uid: string;
  byCategory: Partial<Record<NotificationCategory, boolean>>;
  updatedAt?: string;
}
