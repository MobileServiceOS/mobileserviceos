...
export function Dashboard({
  ...
  const lowStock = useMemo(() => {
    ...
    const top5Tires = (() => {
      ...
      safeJobs.forEach((j) => {
        if (j.tireSize) c[j.tireSize] = (c[j.tireSize] || 0) + Number(j.qty || 1);
      });
    ...
    inv.forEach((i) => {
      const n = normalizeTireSize(i.size);
      if (n) byN[n] = (byN[n] || 0) + Number(i.qty || 0);
    });
  ...
  return (
    <div className="page page-enter">
        ...
        <div className="pending-banner card-anim" onClick={() => setTab('history')}>
          <div>
            <div className="pending-banner-title">
              {pendingJobs.length} Pending Job{pendingJobs.length > 1 ? 's' : ''}
            </div>
      ...
      <div className="pro-hero card-anim">
        <div style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1.4px', color: 'var(--t3)', marginBottom: 12 }}>
          This Week's Gross
        </div>
        <div className="hero-amount">
          <span className="currency">$</span>
          {Math.floor(heroValue).toLocaleString()}
        </div>
        <div className="goal-track">
          <div
            className={'goal-fill ' + (goalPct >= 100 ? 'full' : goalPct >= 50 ? 'mid' : 'low')}
            style={{ width: goalPct + '%' }}
          />
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, fontWeight: 700, color: 'var(--t3)' }}>
          <span>
            {Math.round(goalPct)}% of {money(settings.weeklyGoal || 0)} goal
          </span>
      ...
      <div className="kpi-grid card-anim">
        <div className="kpi">
          ...
          <div className="kpi-value">{todayJobs.length} job{todayJobs.length !== 1 ? 's' : ''}</div>
        </div>
        <div className="kpi">
          <div className="kpi-label">Week Jobs</div>
        ...
        <div className="kpi">
          ...
          <div className="kpi-value">{money(avgProfit)}</div>
        </div>
      </div>
          ...
          <div className="card card-anim">
            <div className="card-pad" style={{ paddingBottom: 8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                ...
                <span className="num" style={{ fontSize: 15, fontWeight: 700, color: 'var(--gold)' }}>
              ...
              {pendingPaymentJobs.slice(0, 5).map((j) => (
                <div
                  key={j.id}
                  style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderTop: '1px solid var(--border2)', gap: 10 }}
                >
                  <div style={{ minWidth: 0, flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--t3)' }}>
                      {j.service} · {fmtDate(j.date)}
                    </div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                    <span className="num" style={{ fontSize: 14, fontWeight: 700, color: 'var(--gold)' }}>
                      {money(j.revenue)}
                    </span>
                    <button className="btn xs success" onClick={() => onMarkPaid(j)}>
                      Paid
                    </button>
                  </div>
                </div>
          ...
          <div className="card-pad">
            ...
            {lowStock.map((ls) => (
              <div key={ls.size} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, padding: '4px 0', color: 'var(--t2)' }}>
                ...
                <span>
                  {ls.onHand} left (sold {ls.soldCount})
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="section-label">Quick Quote</div>
      <div className="quote-box card-anim">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
          ...
          <div className="field" style={{ marginBottom: 0 }}>
            ...
            <select value={qqForm.vehicleType} onChange={(e) => qqChange('vehicleType', e.target.value)}>
              {Object.keys(settings.vehiclePricing || DEFAULT_VEHICLE_PRICING).map((v) => (
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
          {([
            ...
            <button key={k} className={'chip sm' + (qqForm[k] ? ' active' : '')} onClick={() => qqChange(k, !qqForm[k])}>
              {l}
            </button>
          ))}
        </div>
        <div style={{ textAlign: 'center', padding: '12px 0' }}>
          <div className="qq-suggested-price">{money(qqMode === 'suggested' ? quote.suggested : quote.premium)}</div>
          <div style={{ fontSize: 10, color: 'var(--t3)', marginTop: 6 }}>
            Direct costs: {money(quote.directCosts)} · Target profit: {money(quote.targetProfit)}
          </div>
        </div>
        <div className="qq-apply-row">
          <button className={'qq-apply-btn suggested' + (qqMode === 'suggested' ? ' active-sel' : '')} onClick={() => setQqMode('suggested')}>
            {`Suggested\n${money(quote.suggested)}`}
          </button>
          <button className={'qq-apply-btn premium' + (qqMode === 'premium' ? ' active-sel' : '')} onClick={() => setQqMode('premium')}>
            {`Premium\n${money(quote.premium)}`}
          </button>
          <button
            className="qq-apply-btn custom"
            onClick={() =>
              onStartJob({
                ...qqForm,
                revenue: qqMode === 'suggested' ? quote.suggested : quote.premium,
              })
            }
          >
            → Log Job
            ...
            <div className="card-pad">
              {sources.slice(0, 5).map(([name, count]) => (
                <div key={name} className="source-bar">
                  <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--t2)', width: 80, flexShrink: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {name}
                  </span>
                  <div className="source-bar-track">
                    <div className="source-bar-fill" style={{ width: (count / maxSrc) * 100 + '%' }} />
                  </div>
                  <span className="num" style={{ fontSize: 12, fontWeight: 700, color: 'var(--t1)', width: 30, textAlign: 'right', flexShrink: 0 }}>
                    {count}
                  </span>
                </div>
              ))}
