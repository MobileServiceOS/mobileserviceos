...
- Tap to start, tap to stop (auto-stops on silence too)
...
2. Tap → mic permission (first time) → `recorder.start()`. Button
   becomes **🎤 Listening — tap to stop**.
3. Tech speaks. On manual stop OR after the browser detects silence
   (~5 s), `onResult(transcript)` fires.
...
- **🎤 Voice fill** button — full-width, gold-tinted, only rendered
  when `isAIConfigured() && supported`. While `listening`, label
  switches to **🎤 Listening — tap to stop** and tapping calls
  `stop()`. While `parsing`, label is **Parsing…** and the button is
  disabled.
...
- A list of chips, one per non-empty field in `fields`. Each chip:
  ...
  - if the target form field is already populated, an
    **"overwrites: …current value…"** sublabel renders below.
- Footer: **Apply** (primary) and **Cancel** buttons. Apply uses the
  current chip set; Cancel discards everything.
...
- `revenue` → `job.revenue`
- `notes` → `job.note` (replaces; the chip shows the
  "overwrites: …" sublabel if `job.note` is already non-empty, so
  the tech can drop the chip to preserve existing notes)
...
- **Conflicting field already populated** → the chip carries an
  "overwrites: …" sublabel so the tech sees it before Apply.
