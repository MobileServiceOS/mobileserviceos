...
export interface Brand {
  ...
  businessType: string;
  tagline: string;
  // Onboarding / location (optional — populated by the onboarding wizard)
  state?: string;
  mainCity?: string;
...
export interface Job {
  ...
  tireModel?: string;
  tireVendor?: string;
  // "Bought for this job" purchase details
  ...
  // Optional per-job location (set by CityStateSelect in AddJob)
  city?: string;
  state?: string;
  fullLocationLabel?: string;
}
...
export interface Settings {
  ...
  expenses: Expense[];
  // Onboarding-derived defaults
  freeMilesIncluded?: number;
  tireRepairTargetProfit?: number;
  tireReplacementTargetProfit?: number;
  installationDefaults?: { basePrice: number; minProfit: number };
  mileagePricingDefaults?: { costPerMile: number; freeMilesIncluded: number };
}
...
export type TabId = 'dashboard' | 'add' | 'history' | 'customers' | 'payouts' | 'expenses' | 'inventory' | 'settings' | 'success';
