...
export function BrandProvider({ children, user }: { children: ReactNode; user: User }) {
  ...
  useEffect(() => {
    ...
    // Hard timeout — if Firestore never responds, proceed with defaults rather than block UI forever.
    ...
    (async () => {
      try {
        ...
        if (snap.exists() && snap.data().businessId) {
          ...
          // Backfill members doc for existing users who signed up before this
          // structure was added — idempotent (merge) so it's free for users
          // who already have it.
          try {
            await setDoc(
          ...
          // First-time signup. Create everything in dependency order:
          //   1. users/{uid} — needed for security rules to authorize step 2+
          //   2. businesses/{uid} — root business doc with owner reference
          //   3. businesses/{uid}/members/{uid} — owner's membership record
          //   4. businesses/{uid}/settings/main — initial brand/settings
          bId = user.uid;
  ...
  const updateBrand = useCallback(
    async (updates: Partial<Brand>) => {
      if (!businessId || !_db) {
        throw new Error('Not signed in or Firestore unavailable');
      }
      try {
